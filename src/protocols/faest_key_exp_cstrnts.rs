...
pub fn faest_aes_key_exp_fwd<const SIZE: usize>(_m : usize, x: ByteOrBytesArray<SIZE>, mtag : bool, mkey : bool, _Delta : [u8;16]) -> ByteOrBytesArray<ret_size_exp_fwd> {
    ...
    for i in 0..lambda{
        ...
        ByteOrBytesArray::set_at_index(&mut y, i, ByteOrBytesArray::get_at_index(&x, i));
    }
    hax_lib::assert_prop!(hax_lib::forall(|i: usize| i >= y.len() || ByteOrBytesElem::same_variant(&y[i], &x[0])));
    let mut iwd = lambda;
    ...
    for j in nk..((R+1) << 2){
        ...
        if cond {
            ...
            for i in 0..32 {
                ...
                ByteOrBytesArray::set_at_index(&mut y, (j << 5) + i, ByteOrBytesArray::get_at_index(&x, iwd + i));
                //hax_lib::assert!(ByteOrBytesElem::same_variant(&y[(j << 5) + i], &x[0]));
                //y[(j << 5) + i] = x[iwd+i];
                //hax_lib::assert_prop!(hax_lib::forall(|i: usize| i >= y.len() || ByteOrBytesElem::same_variant(&y[i], &x[0])));
            }
            ...
            for i in 0..32{
                ...
                ByteOrBytesArray::set_at_index(&mut y, (j << 5) + i, value_to_pushed);
                //hax_lib::assert!(ByteOrBytesElem::same_variant(&y[(j << 5) + i], &x[0]));
...
// x_k is 1408
#[hax_lib::fstar::options("--z3rlimit 500")]
#[hax_lib::requires(matches!(x, x_k) && !(mtag && mkey) && N >= 320 && M >= 1312 && ByteOrBytesElem::same_variant(Delta, &x[0]))]
    ...
    -> ByteOrBytesArray<ret_size_exp_bwd>
{
    ...
    for j in 0..S_ke{
        ...
        hax_lib::assume!((j << 3) + 8 < x.len());
        hax_lib::assume!(c << 3 <= usize::MAX - 8);
        hax_lib::assume!(i_wd <= usize::MAX - ((c << 3) + 8));
        hax_lib::assume!(i_wd + (c << 3) + 8 < x_k.len());
        ...
        for i in 0..8 {
            ...
            ByteOrBytesArray::set_at_index(&mut x_tilde, i, ByteOrBytesElem::xor_array(&ByteOrBytesArray::get_at_index(&parameter_a, i), &ByteOrBytesArray::get_at_index(&parameter_b, i)));
        }
        ...
        if !mtag && rmvRcon && (c == 0) {
            ...
            for i in 0..8{
                ...
                ByteOrBytesArray::set_at_index(&mut x_tilde, i, ByteOrBytesElem::xor_array(&existing, &r));
        ...
        for i in 0..8usize{
            ...
            hax_lib::assume!(ByteOrBytesElem::same_variant(&parameter_a, &parameter_b));
            let middle_result: ByteOrBytesElem = ByteOrBytesElem::xor_array(&parameter_a, &parameter_b);
            hax_lib::assume!(ByteOrBytesElem::same_variant(&middle_result, &parameter_c));
        ...
        if !mtag {
            ...
            //hax_lib::assume!(ByteOrBytesElem::same_variant(&y_tilde[0], delta_or_1));
        ...
        for i in 0..8{
            ...
            ByteOrBytesArray::set_at_index(&mut y, (j << 3) + i, ByteOrBytesArray::get_at_index(&y_tilde, i));
        }
...
pub fn faest_aes_exp_cstrnts_wv(w : [u8; l_ke], v : [[u8; 16]; l_ke], mkey : bool) -> ([[u8;16]; S_ke], [[u8;16]; S_ke], [u8; 1408], [[u8;16]; 1408] ) {
    ...
    for j in 0..(S_ke >> 2){
        ...
        for r in 0..4 {
            ...
            let k_hat_slice: ByteOrBytesArray<8> = ByteOrBytesArray::Byte(ByteArray(k[(i_wd + (rotated << 3))..(i_wd + (rotated << 3) + 8)].try_into().unwrap()));
            let v_k_hat_slice: ByteOrBytesArray<8> = ByteOrBytesArray::Bytes(BytesArray(v_k[(i_wd + (rotated << 3))..(i_wd + (rotated << 3) + 8)].try_into().unwrap()));
            let w_hat_slice : ByteOrBytesArray<8> = ByteOrBytesArray::Byte(ByteArray(w_tilde[((j << 5) + (r << 3))..((j << 5) + (r << 3) + 8)].try_into().unwrap()));
            let v_w_hat_slice : ByteOrBytesArray<8> = ByteOrBytesArray::Bytes(BytesArray(v_w[((j << 5) + (r << 3))..((j << 5) + (r << 3) + 8)].try_into().unwrap()));
