...
pub fn faest_key_gen() -> ([u8; lambda_bytes], Pk) {
    ...
    loop {
        ...
        for b in 0..beta {
            ...
            for i in 0..s_enc {
                // enc_fwd[i] is [u8; lambda_bytes] — check if the F28 element is zero
                // the F28 element is the first byte of the lambda-bit value
                if enc_fwd[i][0] == 0 && enc_fwd[i][1..].iter().all(|&x| x == 0) {
                    valid = false;
                    break;
                }
            }
