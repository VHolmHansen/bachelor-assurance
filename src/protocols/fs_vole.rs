...
pub fn chall_dec_k0(chall: [u8; 16], i: usize) -> [u8; k_0] {
    ...
    hax_lib::assert_prop!(hax_lib::forall(|i: usize| i >= bits.len() || bits[i] <= 1));
    for idx in 0..12 {
        hax_lib::loop_invariant!(
            hax_lib::Prop::from(idx <= 12)
            .and(hax_lib::Prop::from((lo + idx) >> 3 < 16))
            .and(hax_lib::forall(|j: usize| j >= bits.len()  || bits[j] <= 1))
        );
        //hax_lib::assert_prop!(hax_lib::forall(|j: usize| j >= bits.len() || bits[j] <= 1));
        hax_lib::assert_prop!(hax_lib::forall(|j: usize| j > idx || j >= bits.len()  || bits[j] <= 1));
        let b = lo + idx;
        let byte_index = b >> 3;
        let bit_index = b % 8;
        let val = aes::bitand_mod(chall[byte_index] >> bit_index, 1);
        hax_lib::assert!(val <= 1);
        hax_lib::assert_prop!(hax_lib::implies(
            hax_lib::forall(|i: usize| i >= bits.len() || bits[i] <= 1),
            hax_lib::forall(|j: usize| j >= bits.len() || j == idx || bits[j] <= 1)));
        //let mut newbits = bits;
        bits[idx] = val;
        ...
        hax_lib::assert_prop!(hax_lib::implies(
            hax_lib::Prop::from(
                bits[idx] <= 1).and(
                hax_lib::forall(|j: usize| j > idx || j >= bits.len() || bits[j] <= 1)),
            hax_lib::forall(|k: usize| k >= bits.len() || bits[k] <= 1)));
        //hax_lib::assert_prop!(hax_lib::implies(
        //    hax_lib::Prop::from(val <= 1).and(hax_lib::forall(
        //        |i: usize| i >= bits.len() || bits[i] <= 1)),
        //    hax_lib::forall(|j: usize| j >= newbits.len() || newbits[j] <= 1)));
        //bits = newbits;
        //hax_lib::assert_prop!(hax_lib::implies(hax_lib::forall(|i: usize| i >= newbits.len() || newbits[i] <= 1),
           //hax_lib::forall(|j: usize| j >= bits.len() || bits[j] <= 1)));
        //hax_lib::assert_prop!(hax_lib::implies(val <= 1, hax_lib::forall(|j: usize| j >= bits.len() || bits[j] <= 1)));
        hax_lib::assert_prop!(hax_lib::forall(|i: usize| i >= bits.len() || bits[i] <= 1));
        hax_lib::assert_prop!(hax_lib::forall(|j: usize| j >= idx || j >= bits.len()  || bits[j] <= 1));
    }
    hax_lib::assert_prop!(hax_lib::forall(|i: usize| i >= bits.len() || bits[i] <= 1));
