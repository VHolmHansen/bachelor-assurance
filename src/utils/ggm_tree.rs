...
#[hax_lib::requires(size_pow > 0 && d > 0 && d <= 12)]
...
#[hax_lib::requires(size > 0 && size_pow > 0 && (size_pow >> 1 == 1 || size_pow >> 1 == 2 || size_pow >> 1 == 4 || size_pow >> 1 == 8
...
pub fn get_leaves_from_cop_and_b<const size : usize, const size_pow : usize>(cop : &sized_array_16<size>, iv : [u8;16], b: u64) -> sized_option_array<size_pow> {
    ...
    for c in 0..cop.len() {
        ...
        match match_value {
            Log2Number::one => {cop_helper::<1, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::two => {cop_helper::<2, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::four => {cop_helper::<4, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::eight => {cop_helper::<8, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::sixteen => {cop_helper::<16, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::thirtytwo => {cop_helper::<32, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::sixtyfour => {cop_helper::<64, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::onehundredandtwentyeight => {cop_helper::<128, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::twohundredandfiftysix => {cop_helper::<256, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::fivehundredandtwelve => {cop_helper::<512, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::onethousandandtwentyfour => {cop_helper::<1024, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
            Log2Number::twothousandsandfortyeight => {cop_helper::<2048, size_pow>(&cop[c], &mut leaves, iv, b, &mut current_start, &mut current_end);}
        }
...
#[hax_lib::opaque()]
#[hax_lib::requires(N > 0 && size_pow > 0 && *current_start <= 2048 && *current_end >= 0 && (size_pow >> 1 == 1 || size_pow >> 1 == 2 || size_pow >> 1 == 4 || size_pow >> 1 == 8
                        || size_pow >> 1 == 16 || size_pow >> 1 == 32 || size_pow >> 1 == 64 || size_pow >> 1 == 128
                        || size_pow >> 1 == 256 || size_pow >> 1 == 512 || size_pow >> 1 == 1024 || size_pow >> 1 == 2048))]
#[hax_lib::ensures(|result| leaves.len() == size_pow && *current_start <= 2048 && *current_end >= 0)]
